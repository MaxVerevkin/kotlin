...
internal class SpecializationInterpreter(
    ...
    val specGenericSignature: SpecGenericSignature,
    api: Int = Opcodes.ASM9
) : Interpreter<SpecializationValue>(api) {
    val specializedLoadStore = mutableListOf<Pair<VarInsnNode, GenericIndex>>()
    ...
    override fun newReturnTypeValue(type: Type?): SpecializationValue? {
        if (specGenericSignature.returnGeneric != null) return SpecializationValue(1, specGenericSignature.returnGeneric)
        return newValue(type)
    }

    override fun newParameterValue(isInstanceMethod: Boolean, local: Int, type: Type?): SpecializationValue? {
        if (isInstanceMethod) error("instance methods are not supported yet")
        var argumentIndex = 0
        ...
        specGenericSignature.parameterMapping[argumentIndex]?.let { return SpecializationValue(1, it) }
        return newValue(type)
    }
    ...
    override fun copyOperation(
        ...
        value?.specGenericIndex?.let { specGenericIndex ->
            when (insn.opcode) {
                Opcodes.ALOAD, Opcodes.ASTORE -> specializedLoadStore.add(insn as VarInsnNode to specGenericIndex)
                // TODO: dup and swap
            }
        }
        return value
    ...
    override fun naryOperation(
        ...
        if (insn is MethodInsnNode &&
            ...
            val index = insn.name.substring("specializedTypeDefaultValueMarker".length).toInt()
            return SpecializationValue(1, GenericIndex(index))
        }

        insn.isCallWithSpecializedReturnType()?.let { index ->
            return SpecializationValue(1, GenericIndex(index))
        }
    ...
    override fun merge(
        ...
        if (value1.specGenericIndex != value2.specGenericIndex) error("Cannot merge values of different specializations: ${value1.specGenericIndex} != ${value2.specGenericIndex}")
        return value1
    }
}
 ...
 */
private fun AbstractInsnNode.isCallWithSpecializedReturnType(): Int? {
    if (this !is InvokeDynamicInsnNode ||
        this.bsm.owner != "kotlin/jvm/specialization/BootstrapMethods" ||
        this.bsm.name != "bootstrapSpecializedGeneric"
    ) {
        return null
    }

    val specSignatureStr = this.bsmArgs[2] as String
    val specializedTypeParameters = decodeSpecializedTypeParametersStr(this.bsmArgs[3] as String)

    val returnTypeGenericIndexFocCallee =
        (specSignatureStr.lines().find { it.startsWith("ret=") } ?: return null)
            .substring("ret=".length).toInt()

    val returnTypeSpecializedTo = specializedTypeParameters[returnTypeGenericIndexFocCallee] ?: return null

    if (returnTypeSpecializedTo.startsWith("i:")) {
        return returnTypeSpecializedTo.substring("i:".length).toInt()
    }
